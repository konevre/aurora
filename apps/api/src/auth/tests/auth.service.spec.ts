...
describe('AuthService', () => {
  ...
  it('signIn creates session and tokens', async () => {
    ...
    const res = await service.signIn({ id: 'u1', username: 'john', tokenVersion: 0 }, { ua: 'ua', ip: '127.0.0.1' });

    expect(sessions.create).toHaveBeenCalled();
    expect(tokens.signAccess).toHaveBeenCalled();
    expect(tokens.signRefresh).toHaveBeenCalled();
    expect(res).toEqual({ access: 'access', refresh: 'refresh', accessTtlMs: 60000, refreshTtlMs: 120000 });
  });
  ...
  it('refresh returns new access', async () => {
    ...
    const res = await service.refresh({ userId: 'u1', username: 'john', sid: 'sid1', tokenVersion: 1 });
    expect(tokens.signAccess).toHaveBeenCalled();
  ...
  it('setAuthCookies sets cookies', () => {
    ...
    service.setAuthCookies(response, { access: 'a', refresh: 'r', accessTtlMs: 1000, refreshTtlMs: 2000 });
  ...
  it('clearAuthCookies clears cookies', () => {
    ...
    expect((response.clearCookie as any).mock.calls[0][0]).toBe(ACCESS_COOKIE);
    expect((response.clearCookie as any).mock.calls[1][0]).toBe(REFRESH_COOKIE);
  });
});
