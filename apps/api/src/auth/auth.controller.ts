...
export class AuthController {
  ...
  async signIn(
    ...
    const user = await this.usersService.findByEmailOrUsername(signInDto.emailOrUsername);
    if (!user) {
  ...
  async signOut(@Req() request: Request, @Res({ passthrough: true }) response: Response) {
    const principal = request.user as { userId: string; username?: string; sid: string; tokenVersion?: number };
