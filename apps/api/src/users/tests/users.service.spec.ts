...
describe('UsersService', () => {
  ...
  it('creates user when email and username are unique', async () => {
    ...
    const result = await service.create({ username: 'john', email: 'john@mail.com', password: 'pass' });
    expect(prisma.user.findFirst).toHaveBeenCalled();
    expect(bcrypt.hash).toHaveBeenCalled();
    expect(prisma.user.create).toHaveBeenCalled();
    expect(result).toEqual({ id: 'u1', username: 'john', email: 'john@mail.com', tokenVersion: 0 });
  });

  it('throws ConflictException if user already exists', async () => {
    prisma.user.findFirst.mockResolvedValue({ id: 'exists' });
    await expect(
      service.create({ username: 'john', email: 'john@mail.com', password: 'pass' })
